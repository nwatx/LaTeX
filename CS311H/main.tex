...
\section{Lecture--September 27, 2022}
...
\begin{proof}
	...
	It can never be the case that $x + y = \frac{1}{3}$ because $x$ and $y$ are integers.
	Therefore we have reached a contradiction and the function is not injective.
\end{proof}
...
\begin{definition}[Bijective Functions]
	\begin{itemize}
		\item Function that is both onto and one-to-one is called a \textbf{bijection}
		\item One-to-one correspondence or invertible function are synonymous.
	\end{itemize}
\end{definition}
...
Why does inverse not exist if $f$ is not surjective? There are elements in the new domain
that would not map.

Composition: $f(x)=2x+3$ and $g(x)=3x+2$ then $f\circ g = 2(3x + 2) + 3 = 6x + 8$.

Another composition: prove that $f^{-1}\circ f$ is the identity function.
You swap the domain and codomain twice.
...
\begin{definition}[Floor and Ceiling Functions]
	Both floor and ceiling functions map $f: \mathbb{R} \to \mathbb{Z}$.

	\begin{itemize}
		\item \textbf{Floor function} $f(x) = \lfloor x \rfloor$ is the largest integer less than or equal to $x$.
		\item \textbf{Ceiling function} $f(x) = \lceil x \rceil$ is the smallest integer greater than or equal to $x$.
	\end{itemize}

	$x=y+\epsilon$ for some $\epsilon \in [0, 1)$.
...
\begin{example}
	...
	LHS:
	\begin{align*}
		\floor{x+k} = \floor{y+\epsilon + k} = y + k
	\end{align*}
\end{example}
...
\begin{proof}
	...
	% \[
	% 	\floor{y + \epsilon} + \floor{y + \epsilon + \frac{1}{2}} \\
	% \]

	\[
		\begin{cases}
			\epsilon \geq 0.5 & \text{then } \floor{y+\epsilon} + \floor{y+\epsilon+\frac{1}{2}} = y + y + 1 = 2y+1 \\
			\epsilon < 0.5    & \text{then } \floor{y+\epsilon} + \floor{y+\epsilon+\frac{1}{2}} = y + y = 2y
		\end{cases}
	\]
\end{proof}
...
\begin{example}
	...
	% 		       & q=1         & q=2         & q=3         & \ldots \\
	% 		\hline
	% 		p=1    & \frac{1}{1} & \frac{1}{2} & \frac{1}{3} & \ldots \\
	% 		p=2    & \frac{2}{1} & \frac{2}{2} & \frac{2}{3} & \ldots \\
	% 		p=3    & \frac{3}{1} & \frac{3}{2} & \frac{3}{3} & \ldots \\
	% 		\vdots & \vdots      & \vdots      & \vdots      & \ddots
	% 	\end{array}
	% \]
	\include{examples/cantor}
